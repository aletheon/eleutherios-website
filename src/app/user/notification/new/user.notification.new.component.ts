...
@Component({
  ...
  templateUrl: './user.notification.new.component.html',
  styleUrls: ['./user.notification.new.component.css']
})
export class UserNotificationNewComponent implements OnInit, OnDestroy, AfterViewInit  {
  ...
  removeTag (notificationTag) {
    this.userNotificationTagService.exists(this.notificationGroup.get('uid').value, this.notificationGroup.get('notificationId').value, notificationTag.tagId).then(exists => {
      if (exists)
        this.userNotificationTagService.delete(this.notificationGroup.get('uid').value, this.notificationGroup.get('notificationId').value, notificationTag.tagId);
    });
  }
  ...
  private initForm () {
    ...
    const runOnceSubscription = this.notification
      .subscribe(notification => {
        if (notification){
            ...
            try {
              ...
              that._notificationTagSubscription = that.userNotificationTagService.getTags(notification.uid, notification.notificationId)
                .subscribe(notificationTags => {
                  ...
                  // don't do search first time through as we don't have any tags yet
                  if (that._firstTimeThrough == false)
                    that.getSearchList();
                }
              );
  ...
  getSearchList (key?: any) {
    ...
    if (this.notificationGroup.get('type').value == 'Forum') {
      ...
      this._searchSubscription = this.serviceService.getServices(this.numberOfItems, key, this._tempNotificationTags, this._tempNotificationTags.length > 0 ? true : false, true, this.notificationGroup.get('paymentType').value).pipe(
        switchMap(services => {
