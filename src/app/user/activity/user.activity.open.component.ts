...
export class UserActivityOpenComponent implements OnInit, OnDestroy {
  ...
  removeActivity (activity) {
    this.userActivityService.delete(this.userId, activity.forumId);
  }

  changeReceivePosts (activity) {
    this.userActivityService.update(this.userId, activity.forumId, { receivePosts: !activity.receivePosts } );
  }
  ...
  ngOnInit () {
    ...
    this._userSubscription = this.auth.user.pipe(take(1)).subscribe(user => {
      if (user){
        ...
        this._subscription = this.userActivityService.getActivities(this.userId).pipe(
