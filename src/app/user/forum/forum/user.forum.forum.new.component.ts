        ...
        .then(forum => {
          if (forum){
            if (forum.title.length > 0){
              ...
              this.router.navigate(['/user/forum/detail'], { queryParams: { userId: this.parentForumUserId, forumId: this.parentForumId } });
            }
          }
          else {
            const snackBarRef = this.snackbar.openFromComponent(
      ...
      if (forum){
        let load = async function(){
          try {
            ...
            // default forum image
            that._defaultForumImageSubscription = that.userForumImageService.getDefaultForumImages(forum.uid, forum.forumId)
              .subscribe(forumImages => {
                if (forumImages && forumImages.length > 0)
                  that.defaultForumImage = of(forumImages[0]);
                else {
                  let tempImage = {
