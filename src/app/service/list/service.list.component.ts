...
export class ServiceListComponent implements OnInit, OnDestroy {
  ...
  getServicesList (key?: any) {
    ...
    if (this.searchServiceCtrl.value && this.searchServiceCtrl.value.length > 0){
      ...
      this._subscription = this.serviceService.getServicesSearchTerm(this.auth.uid, this.numberItems, key, this._tempSearchTags, this.includeTagsInSearch, false, this.serviceGroup.get('paymentType').value, this.serviceGroup.get('currency').value, this.serviceGroup.get('startAmount').value, this.serviceGroup.get('endAmount').value).pipe(
        switchMap(services => {
      ...
      this._subscription = this.serviceService.getAllServices(this.auth.uid, this.numberItems, key, this._tempSearchTags, this.includeTagsInSearch, false, this.serviceGroup.get('paymentType').value, this.serviceGroup.get('currency').value, this.serviceGroup.get('startAmount').value, this.serviceGroup.get('endAmount').value).pipe(
        switchMap(services => {
          if (services && services.length > 0){
            let observables = services.map(service => {
              if (service){
