    ...
    if (event.type == 'account.application.authorized'){
      ...
      if (paymentSnapshot.exists){
        ...
        await admin.firestore().collection(`users/${payment.sellerUid}/receipts`).doc(payment.receiptId).set({
          ...
          amount: payment.amount,
          currency: payment.currency,
          buyerServiceType: payment.buyerServiceType,
          buyerServiceTitle: payment.buyerServiceTitle,
          buyerServiceDescription: payment.buyerServiceDescription,
          sellerServiceType: payment.sellerServiceType,
          sellerServiceTitle: payment.sellerServiceTitle,
          sellerServiceDescription: payment.sellerServiceDescription,
          quantity: payment.quantity,
          status: 'Pending',
          buyerUid: payment.buyerUid,
          buyerServiceId: payment.buyerServiceId,
...
exports.createPaymentIntent = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    const newPayment = {
      ...
      buyerServiceType: buyerService.type,
      buyerServiceTitle: buyerService.title,
      buyerServiceDescription: buyerService.description,
      sellerServiceType: sellerService.type,
      sellerServiceTitle: sellerService.title,
      sellerServiceDescription: sellerService.description,
      quantity: 1,
      status: '',
      buyerUid: buyerUid,
